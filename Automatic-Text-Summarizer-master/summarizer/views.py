...
from .algorithms.scoring import scoring_algorithm, scoring_Hindi
from .algorithms.frequency import extraction, frequency_Hindi, frequency_algorithm
...
def summarize_hindi_page(request):
    url = request.GET.get('url')
    long_text = request.GET.get('long-text')
    sentence_no = int(request.GET.get('number'))
    algorithm = request.GET.get('algorithm')
    ...
    if algorithm == '1':
        result_list = scoring_Hindi.scoring_main(long_text, sentence_no)
    elif algorithm == '2':
        result_list = frequency_Hindi.frequency_main_Hindi(long_text, sentence_no)
